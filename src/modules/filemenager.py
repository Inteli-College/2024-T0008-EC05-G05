...
class HydroDB:
    ...
    def quarry(self, table:str, columns:list=None, filter:str=None):
        ...
        # if tables is str:
        #     tables = _to_list(tables)
        
        # if columns is list:
        #     columns = _to_tuple(columns)

        # if filters is list and filters is None:
        #     filters = _to_tuple(filters)
...
def _match_filter_command(table_to_filter:dict , command:str, columns:list) -> dict:
    ...
    # print(f"COLUMN TO GUET : '{column}' \nOPERATOR : '{operator}' \nVALUE TO FILTER : '{value_to_filter}'\n\n")
    # print(f"TABLE TO FILTER =>\n{table_to_filter}")

    for row in table_to_filter:
        filtered_table:dict = {}
        # print(f"\n\nROW ==>\n{row}")
        if column in row: 
            text_marker(f"COLUMN TO WATCH => {column} ||| WITH VALUE => {value_to_filter}", ".")       
            match operator:
                case "=":
                    if value_to_filter == row[column]:
                        text_marker(f"KEY VALUE =>'{row[column]}'")

                        for i in columns:
                            filtered_table.update({i : row[i]})
